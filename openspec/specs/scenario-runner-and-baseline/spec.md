...
The scenario runner and baseline capability provides a deterministic scenario execution path for experiment runs. It defines immutable scenario inputs, generates ordered scenario steps for steady, ramp, and burst profiles, runs those steps against a fixed baseline workload executor, and records metrics evidence through the existing observation layer. It is intentionally non-adaptive: no scale decisions, no executor resizing, and no queue mutation occur in this capability.
