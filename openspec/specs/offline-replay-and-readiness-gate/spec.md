# Offline Replay and Readiness Gate Specification

## Purpose

The offline replay and readiness gate capability provides a strictly read-only analysis layer for experiment runs. It validates baseline run evidence for structural integrity, replays each snapshot through a fixed set of three threshold-policy configurations (default, conservative, aggressive) to produce deterministic replay decision evidence, aggregates the results into per-run and per-scenario summaries with action/gate counts and oscillation signals, compares the three configurations against the default baseline, assesses mutation readiness with a fixed profile-completeness and risk-threshold rule set, and emits controlled JSON / Markdown report artifacts under `outputs/reports/v0.4.0/`. It does not import or invoke runtime mutation, queue resizing, scheduling, persistence, or external API types, and never reads wall-clock time.
