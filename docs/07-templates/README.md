# Templates

This directory stores reusable templates for future version design, ADR, and change decomposition work.

## Documents

1. `docs/07-templates/version-design-template.md`
2. `docs/07-templates/architecture-decision-record-template.md`
3. `docs/07-templates/change-decomposition-template.md`
4. `docs/07-templates/change-delivery-checklist-template.md`
5. `docs/07-templates/managed-change-stage-package-template.md`
6. `docs/07-templates/retrospective-template.md`
